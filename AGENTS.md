...
This project changes one thing: `src/usbComms.c`, to move the transport off
libusb and onto the internal transport.

Formatting is `uncrustify.cfg`, run by hand with `./do-uncrustify`.

Build: Xcode project `G2 Editor.xcodeproj`. The third-party dependencies
(glfw, freetype, libusb) are built by hand. See `README.md`.

This repository registers no tests, by decision.
